"""Stage Creator Mode selected-entity display-label rename proposals."""
...
from .creator_entity_rename_live_ops import build_creator_entity_rename_live_ops
from .creator_entity_rename_request import CreatorEntityRenameRequest


@dataclass(frozen=True, slots=True)
class CreatorEntityRenameStagingResult:
    """Result for staging a display-label rename proposal."""
...
def stage_creator_entity_rename_proposal(
    request: CreatorEntityRenameRequest,
    bridge: object | None,
) -> CreatorEntityRenameStagingResult:
    """Stage a display-label rename proposal through the official bridge."""

    live_ops = build_creator_entity_rename_live_ops(request)
    if not live_ops.ok:
    ...
    if result.get("ok") is not True:
        return CreatorEntityRenameStagingResult(
            ok=False,
            errors=(_bridge_message(result, fallback="Failed to stage rename proposal."),),
            warnings=tuple(live_ops.warnings),
        )
