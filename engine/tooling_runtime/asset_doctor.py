...
def _discover_scene_paths(repo_root: Path) -> list[Path]:
    ...
    if packs_dir.exists():
        for scenes_folder in sorted([p for p in packs_dir.rglob("scenes") if p.is_dir()], key=lambda x: x.as_posix()):
...
def doctor_assets(
    *,
    ...
    def add_issue(severity: str, code: str, path: Path | str, message: str) -> None:
        entry = _Entry(code=code, path=_normalize_path_for_json(path, repo_root=repo_root), message=_single_line(message))
        if severity == "error":
            errors.append(entry)
        else:
            warnings.append(entry)

    def add_fix(code: str, path: Path | str, message: str) -> None:
        fixes.append(_Entry(code=code, path=_normalize_path_for_json(path, repo_root=repo_root), message=_single_line(message)))

    def parse_issue_severity() -> str:
        return "error" if strict else "warning"

    # 1) Missing required files.
    config_path = repo_root / "config.json"
    if not config_path.is_file():
        add_issue("error", "required.config_json.missing", "config.json", "missing required file config.json at repo root")

    quests_path = repo_root / "assets" / "data" / "quests.json"
    if not quests_path.is_file():
        add_issue("error", "required.quests_json.missing", quests_path, "missing required file assets/data/quests.json")

    events_path = repo_root / "assets" / "data" / "events.json"
    if not events_path.is_file():
        add_issue("error", "required.events_json.missing", events_path, "missing required file assets/data/events.json")

    # Track all JSON files we successfully parsed so we can optionally add trailing newline fixes.
    ...
    for scene_path in scene_paths:
        ...
        if not ok:
            add_issue(parse_issue_severity(), "scene.json.invalid", scene_path, err)
            continue
        parsed_json[scene_path] = payload

    # 2) Cross-reference checks (worlds -> scenes).
    known_scene_ids: dict[str, Path] = {}
    for world_path in _discover_world_paths(repo_root):
        ok, world_payload, err = _try_read_json(world_path)
        if not ok:
            add_issue(parse_issue_severity(), "world.json.invalid", world_path, err)
            continue
        parsed_json[world_path] = world_payload
        if not isinstance(world_payload, dict):
            add_issue("error", "world.schema.invalid", world_path, "world file must be a JSON object")
            continue

        scenes = world_payload.get("scenes")
        if not isinstance(scenes, dict):
            add_issue("error", "world.scenes.invalid", world_path, "world.scenes must be an object mapping scene ids")
            continue

        for raw_id, entry in scenes.items():
            scene_id = _norm_key(raw_id)
            if scene_id is None or scene_id in known_scene_ids:
                continue
            path = entry.get("path") if isinstance(entry, dict) else None
            if isinstance(path, str) and path.strip():
                known_scene_ids[scene_id] = (repo_root / path).resolve()
        ...
        for scene_id in sorted(scenes.keys(), key=lambda s: (str(s).strip().lower(), str(s))):
            ...
            if not isinstance(entry, dict):
                add_issue("error", "world.scene.entry.invalid", world_path, f"world.scenes[{scene_id!r}] must be an object")
                continue
            ...
            if _is_missing_text(scene_ref):
                ...
                continue
            scene_file = repo_root / str(scene_ref)
            if not scene_file.exists():
                add_issue(
                    "error",
                    "world.scene.file.missing",
                    str(scene_ref),
                    f"referenced by {_normalize_path_for_json(world_path, repo_root=repo_root)} (scene id {scene_id!r})",
                )

    # 2) Cross-reference checks in scenes (TriggerZone / SceneTransition).
    for scene_path in scene_paths:
        payload = parsed_json.get(scene_path)
                    ...
                    if _behaves_as_path(target_str):
                        ...
                        if not target_path.exists():
                            add_issue(
                                ...
                                target_str,
                                f"referenced by {_normalize_path_for_json(scene_path, repo_root=repo_root)} (entity index {idx})",
                            )
                        else:
                            ...
                            if not ok:
                                add_issue(
                                    ...
                                    target_path,
                                    f"referenced by {_normalize_path_for_json(scene_path, repo_root=repo_root)} (entity index {idx}): {err}",
                                )
                            else:
                                parsed_json[target_path] = target_payload
                    else:
                        target_id = _norm_key(target_str)
                        if target_id is not None and target_id not in known_scene_ids:
                            add_issue(
                                "warning",
    ...
    # 3) Duplicate ids (quests/events).
    if quests_path.is_file():
        ok, quests_payload, err = _try_read_json(quests_path)
        if not ok:
            add_issue(parse_issue_severity(), "quests.json.invalid", quests_path, err)
        else:
            parsed_json[quests_path] = quests_payload
            quest_ids: list[str] = []
            ...
            for qid in sorted([k for k, v in counts.items() if v > 1]):
                add_issue("error", "quests.id.duplicate", quests_path, f"duplicate quest id: {qid}")

    if events_path.is_file():
        ok, events_payload, err = _try_read_json(events_path)
        if not ok:
            add_issue(parse_issue_severity(), "events.json.invalid", events_path, err)
        else:
            parsed_json[events_path] = events_payload
            ...
            for name in sorted([k for k, v in counts.items() if v > 1]):
                add_issue("error", "events.name.duplicate", events_path, f"duplicate event name: {name}")

    # Fix: trailing newline for tooling-written JSON files (safe-only).
    if fix:
        for path in sorted(parsed_json.keys(), key=lambda p: _normalize_path_for_json(p, repo_root=repo_root)):
            _maybe_fix_trailing_newline(path, payload=parsed_json[path], fixes=fixes, repo_root=repo_root)

    missing_prefab_assets: list[dict[str, str]] = []
    missing_prefab_assets_warnings: list[dict[str, str]] = []
    pack_scope = sorted({str(p).strip() for p in (packs or []) if str(p).strip()})
    ...
    try:
        ...
        image_cache_path = repo_root / ".mesh" / "cache" / "image_sizes.json"
        ...
        def _normalize_asset_path(path_value: str | Path) -> str:
            return _normalize_path_for_json(path_value, repo_root=repo_root)
                ...
                if image_path is not None:
                    ...
                    if fw > img_w or fh > img_h:
                        warn_prefab_asset(
                            ...
                            "entity.sprite_sheet",
                            image_path,
                            f"frame dimensions exceed image size ({fw}x{fh} > {img_w}x{img_h})",
                        )
                        continue
    ...
    errors_sorted = sorted([e.to_dict() for e in errors], key=lambda d: (d["path"], d["code"], d["message"]))
    warnings_sorted = sorted([e.to_dict() for e in warnings], key=lambda d: (d["path"], d["code"], d["message"]))
    fixes_sorted = sorted([e.to_dict() for e in fixes], key=lambda d: (d["path"], d["code"], d["message"]))
