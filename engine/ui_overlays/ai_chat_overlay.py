...
class AIChatOverlay(UIElement):
    ...
    def draw(self) -> None:
        ...
        content_header_top = dock.top - TAB_HEADER_HEIGHT
        _draw_tb_rectangle_filled(dock.left, dock.right, content_header_top, dock.bottom, EDITOR_THEME.panel_strong_bg)
        _draw_tb_rectangle_outline(dock.left, dock.right, content_header_top, dock.bottom, EDITOR_THEME.panel_strong_border, 1)

        content_top = content_header_top - PADDING
        draw_text_cached(
            "AI Chat",
            dock.left + PADDING,
    ...
    def on_mouse_press(self, x: float, y: float, button: int, modifiers: int = 0) -> bool:  # noqa: ARG002
        ...
        if chat is None:
            return False
        if not self._point_in_content_rect(float(x), float(y)):
            return False
    ...
    def _point_in_content_rect(self, x: float, y: float) -> bool:
        controller = getattr(self.window, "editor_controller", None)
        ...
        left_w, right_w = get_effective_dock_widths(controller, window_w)
        dock = compute_editor_shell_layout(window_w, window_h, left_w, right_w).right_dock
        return dock.left <= x <= dock.right and dock.bottom <= y <= dock.top - TAB_HEADER_HEIGHT
