...
def test_entity_paste_integrates_undo_redo(capsys) -> None:
    ...
    try:
        window = type(
            ...
            (),
            {
                ...
                "input_controller": type("IC", (), {"mouse_x": 50.0, "mouse_y": 60.0})(),
        ...
        assert "ENTITY_PASTE ok count=1" in capsys.readouterr().out.strip()
        assert any(e["id"] == "a__paste0" for e in sc.get_authored_scene_payload()["entities"])
        ...
        assert not any(e["id"] == "a__paste0" for e in sc.get_authored_scene_payload()["entities"])
