...
def test_help_overlay_persists_across_scene_ui_rebuild(monkeypatch) -> None:
    ...
    window.player_hud = PlayerHUD(window)  # type: ignore[arg-type]
    window.game_over_screen = GameOverScreen(window)  # type: ignore[arg-type]
    window.pause_menu = PauseMenu(window)  # type: ignore[arg-type]
    window.help_overlay = HelpOverlay(window)  # type: ignore[arg-type]
