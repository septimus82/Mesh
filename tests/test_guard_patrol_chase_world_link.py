...
def test_guard_patrol_chase_demo_is_reachable_from_upper_hall() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    world = json.loads((repo_root / "worlds" / "main_world.json").read_text(encoding="utf-8"))

    assert "guard_patrol_chase_demo" in world["scenes"]
    assert world["scenes"]["guard_patrol_chase_demo"]["path"].replace("\\", "/") == "scenes/guard_patrol_chase_demo.json"

    links = world["links"]
    assert {
        "from": "upper_hall",
        "to": "guard_patrol_chase_demo",
        "via": "GuardPatrolChaseDemoDoor",
    } in links
    assert {
        "from": "guard_patrol_chase_demo",
        "to": "upper_hall",
        "via": "GuardPatrolChaseDemoReturnDoor",
    } in links

    upper_hall = json.loads((repo_root / "scenes" / "upper_hall.json").read_text(encoding="utf-8"))
    demo = json.loads((repo_root / "scenes" / "guard_patrol_chase_demo.json").read_text(encoding="utf-8"))

    door = _find_entity_by_name(upper_hall, "GuardPatrolChaseDemoDoor")
    assert door is not None
    cfg_root = door.get("behaviour_config")
    cfg = cfg_root.get("SceneTransition") if isinstance(cfg_root, dict) else None
    assert isinstance(cfg, dict)
    assert cfg.get("target_scene") == "scenes/guard_patrol_chase_demo.json"
    assert cfg.get("spawn_id") == "default"
    assert "default" in _scene_spawn_ids(demo)

    back = _find_entity_by_name(demo, "GuardPatrolChaseDemoReturnDoor")
