"""Integration tests for Episode 03: The Three Sigils."""
...
QUEST_ID = "episode_03_ep03"
INTRO_CUTSCENE_ID = "ep03_intro"
RESET_CUTSCENE_ID = "ep03_reset_wrong"
OUTRO_CUTSCENE_ID = "ep03_outro"
...
def _build_episode_context(*, start_quest: bool = True) -> dict[str, Any]:
    ...
    reset_runner = _build_cutscene_runner(window, _load_cutscene_script(RESET_CUTSCENE_ID))
...
def _drain_and_route(ctx: dict[str, Any]) -> list[Any]:
    ...
    for event in events:
        ...
        if event.event_type == "ep03_intro_start" and not ctx["intro_runner"].is_running:
            ctx["intro_runner"].start()
        elif event.event_type == "ep03_reset_start" and not ctx["reset_runner"].is_running:
            ctx["reset_runner"].start()
        elif event.event_type == "ep03_outro_start" and not ctx["outro_runner"].is_running:
...
def _advance(ctx: dict[str, Any], dt: float) -> list[Any]:
    for cutscene in (ctx["intro_runner"], ctx["reset_runner"], ctx["outro_runner"]):
...
def _interact(ctx: dict[str, Any], interactable_id: str) -> list[Any]:
    player = ctx["entities"]["episode_03_ep03_player"]
    ...
    entities_payload.append(
        {
            ...
            "behaviour_state": {
                ...
                "reset": ctx["reset_runner"].saveable_state(),
...
def _restore_context(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    ctx["reset_runner"].restore_state(snapshot["reset"])
...
def _run_episode(
    *,
    choice: str,
    use_hint: bool,
    wrong_order_first: bool,
    save_restore_mid: bool,
) -> dict[str, Any]:
    ...
    record()
    _step_on_trigger(
        ctx["trigger_volumes"]["episode_03_ep03_entry_trigger"],
        ctx["entities"]["episode_03_ep03_player"],
    )
    ...
    mentor_dialogue = ctx["dialogue_runners"]["episode_03_ep03_mentor"]
    assert mentor_dialogue.is_running is True
    assert mentor_dialogue.choose(0 if choice == "help" else 1) is True
    _drain_until_empty(ctx)
    record()

    if use_hint:
        _interact(ctx, "episode_03_ep03_hint_plaque")
        record()

    if wrong_order_first:
        _interact(ctx, "episode_03_ep03_sigil_b")
        _advance(ctx, 0.6)
        record()

    _interact(ctx, "episode_03_ep03_sigil_a")
    if save_restore_mid:
    ...
    _interact(ctx, "episode_03_ep03_sigil_b")
    _interact(ctx, "episode_03_ep03_sigil_c")
    record()

    _interact(ctx, "episode_03_ep03_exit_door")
    _advance(ctx, 0.3)
    record()
...
class TestEpisode03Integration:
    def test_episode_03_hints_enabled_happy_path(self) -> None:
        ctx = _build_episode_context()
        ...
        _interact(ctx, "episode_03_ep03_hint_plaque")
        assert _get_flag(ctx, "ep03.hints_enabled") is True
        assert _get_flag(ctx, "ep03.no_hints") is not True
        assert _get_flag(ctx, "ep03.hint_seen") is True
        assert any(evt["event_type"] == "ep03_hint_shown" for evt in ctx["event_log"])

        _interact(ctx, "episode_03_ep03_sigil_a")
        _interact(ctx, "episode_03_ep03_sigil_b")
        _interact(ctx, "episode_03_ep03_sigil_c")

        assert _get_flag(ctx, "ep03.seq1") is True
        assert _get_flag(ctx, "ep03.seq2") is True
        assert _get_flag(ctx, "ep03.solved") is True
        assert _get_flag(ctx, "ep03.exit_unlocked") is True

        door_entity = ctx["entities"]["episode_03_ep03_exit_door"]
        ...
        assert _get_flag(ctx, "ep03.complete") is True
        assert any(evt["event_type"] == "quest_ep03_complete" for evt in ctx["event_log"])

    def test_episode_03_no_hints_happy_path(self) -> None:
        ctx = _build_episode_context()
        _step_on_trigger(
            ctx["trigger_volumes"]["episode_03_ep03_entry_trigger"],
            ctx["entities"]["episode_03_ep03_player"],
        ...
        assert _get_flag(ctx, "ep03.no_hints") is True
        assert _get_flag(ctx, "ep03.hints_enabled") is not True
        ...
        assert quest_state is not None
        assert quest_state.status == "completed"
        assert _get_flag(ctx, "ep03.hint_seen") is not True
        assert any(evt["event_type"] == "quest_ep03_complete" for evt in ctx["event_log"])

    def test_episode_03_wrong_order_triggers_reset_then_can_solve(self) -> None:
        ctx = _build_episode_context()
        _step_on_trigger(
            ctx["trigger_volumes"]["episode_03_ep03_entry_trigger"],
            ctx["entities"]["episode_03_ep03_player"],
        ...
        assert _get_flag(ctx, "ep03.seq1") is not True
        assert _get_flag(ctx, "ep03.seq2") is not True
        assert any(evt["event_type"] == "ep03_wrong_order" for evt in ctx["event_log"])
        assert any(evt["event_type"] == "ep03_reset_start" for evt in ctx["event_log"])
        assert any(evt["event_type"] == "ep03_reset_done" for evt in ctx["event_log"])
    ...
    def test_episode_03_save_restore_mid_puzzle(self) -> None:
        ctx = _build_episode_context()
        _step_on_trigger(
            ctx["trigger_volumes"]["episode_03_ep03_entry_trigger"],
            ctx["entities"]["episode_03_ep03_player"],
        ...
        assert _get_flag(restored, "ep03.seq1") is True
        assert _get_flag(restored, "ep03.seq2") is not True

        _interact(restored, "episode_03_ep03_sigil_b")
        _interact(restored, "episode_03_ep03_sigil_c")
        _interact(restored, "episode_03_ep03_exit_door")
    ...
    def test_episode_03_determinism_digest_and_events(self) -> None:
        first = _run_episode(choice="help", use_hint=True, wrong_order_first=False, save_restore_mid=True)
        second = _run_episode(choice="help", use_hint=True, wrong_order_first=False, save_restore_mid=True)
