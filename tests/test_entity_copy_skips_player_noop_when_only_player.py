...
def test_entity_copy_skips_player_noop_when_only_player(capsys) -> None:
    ...
    try:
        ...
        sc._loaded_scene_source_data = {
            "entities": [
                {"id": "player_1", "prefab_id": "player", "x": 1.0, "y": 2.0, "tags": ["player"]},
            ]
        }
        window.scene_controller = sc
        window.entity_select_state = EntitySelectState(selected_ids=["player_1"], primary_id="player_1")
        ...
        assert capsys.readouterr().out.strip() == "ENTITY_COPY noop reason=only_player"
        assert getattr(window, "entity_clipboard", None) is None
    finally:
