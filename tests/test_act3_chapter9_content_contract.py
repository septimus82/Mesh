...
ACT3_CH9_SCENES = [
    "packs/core_regions/scenes/Act3_Chapter9_Entry.json",
    "packs/core_regions/scenes/Act3_Chapter9_Briefing.json",
    "packs/core_regions/scenes/Act3_Chapter9_Exit.json",
]

ACT3_CH9_QUEST_IDS = {
    "quest_act3_ch9_entry",
    "quest_act3_ch9_handoff",
    "quest_act3_ch9_complete",
}
...
FORWARD_CHAIN = [
    (
        ...
        "ToAct3Chapter9Entry",
        "packs/core_regions/scenes/Act3_Chapter9_Entry.json",
        "act3_chapter8_complete",
    ),
    (
        "packs/core_regions/scenes/Act3_Chapter9_Entry.json",
        "ToAct3Chapter9Briefing",
        "packs/core_regions/scenes/Act3_Chapter9_Briefing.json",
        "act3_ch9_started",
    ),
    (
...
def test_act3_ch9_scene_files_parse_and_references_resolve() -> None:
    for scene_path in ACT3_CH9_SCENES:
        path = Path(scene_path)
        assert path.exists(), scene_path
        _assert_scene_references_resolve(scene_path, _load_json(scene_path))


def test_act3_ch9_world_registry_contains_new_scenes() -> None:
    world = _load_json("worlds/act3_chapter1_stub.json")
    scenes = world.get("scenes")
    assert isinstance(scenes, dict)
    for key in ["act3_chapter9_entry", "act3_chapter9_briefing", "act3_chapter9_exit"]:
...
def test_act3_ch9_quests_exist_and_have_objective_lines() -> None:
    ...
    for quest_id in ACT3_CH9_QUEST_IDS:
...
def test_act3_ch9_briefing_beats_exist_with_exact_toasts() -> None:
    scene = _load_json("packs/core_regions/scenes/Act3_Chapter9_Briefing.json")
    toasts = {
        cfg.get("toast")
        for cfg in _set_configs(scene)
        if isinstance(cfg.get("toast"), str)
    }
    assert "The relay is quiet. Too quiet." in toasts
    assert "A sealed directive routes you deeper into the grid." in toasts
    assert "Objective: reach the next sector uplink." in toasts


def test_act3_ch9_commit_zone_exists_sets_mid_arc_complete_with_exact_toast_and_once() -> None:
    scene = _load_json("packs/core_regions/scenes/Act3_Chapter9_Briefing.json")
    cfg = _zone_setter(scene, zone_name="Act3Chapter9CommitZone", flag_name="act3_mid_arc_complete")
    assert isinstance(cfg, dict)
    assert cfg.get("once") is True
    assert cfg.get("toast") == "Handoff accepted."
    assert cfg.get("toast_seconds") == 2.0


def test_act3_ch9_backup_commit_exists_with_same_toast_and_flag() -> None:
    scene = _load_json("packs/core_regions/scenes/Act3_Chapter9_Briefing.json")
    cfg = _zone_setter(scene, zone_name="Act3Chapter9BackupCommitZone", flag_name="act3_mid_arc_complete")
    assert isinstance(cfg, dict)
    assert cfg.get("once") is True
    assert cfg.get("toast") == "Handoff accepted."
    assert cfg.get("toast_seconds") == 2.0
...
def test_act3_ch9_exit_transition_requires_mid_arc_complete() -> None:
    scene = _load_json("packs/core_regions/scenes/Act3_Chapter9_Briefing.json")
    transition = _find_entity(scene, name="ToAct3Chapter9Exit")
    assert isinstance(transition, dict)
    require_flags = transition.get("require_flags")
    assert isinstance(require_flags, list)
    assert require_flags == ["act3_mid_arc_complete"]


def test_act3_ch9_checkpoint_exists_with_expected_toast_once_and_seconds() -> None:
    scene = _load_json("packs/core_regions/scenes/Act3_Chapter9_Exit.json")
    cfg = _zone_setter(scene, zone_name="Act3Chapter9CheckpointZone", flag_name="act3_ch9_checkpoint")
    assert isinstance(cfg, dict)
    assert cfg.get("once") is True
    assert cfg.get("toast") == "Checkpoint reached: Act 3 Chapter 9"
    assert cfg.get("toast_seconds") == 2.0


def test_act3_ch9_forward_path_is_structurally_valid_and_uses_in_chapter_flags() -> None:
    entry = _load_json("packs/core_regions/scenes/Act3_Chapter9_Entry.json")
    briefing = _load_json("packs/core_regions/scenes/Act3_Chapter9_Briefing.json")
    started_cfg = _zone_setter(entry, zone_name="Act3Chapter9EntryBriefingZone", flag_name="act3_ch9_started")
    handoff_cfg = _zone_setter(briefing, zone_name="Act3Chapter9CommitZone", flag_name="act3_mid_arc_complete")
    assert isinstance(started_cfg, dict)
    assert isinstance(handoff_cfg, dict)
