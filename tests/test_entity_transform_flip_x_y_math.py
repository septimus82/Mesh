...
def test_entity_transform_flip_x_y_math(capsys) -> None:
    ...
    try:
        ...
        sc._loaded_scene_source_data = {
            "entities": [
                ...
                {"id": "b", "prefab_id": "slime_blob", "x": 2.0, "y": 4.0},
            ]
        }
        ...
        assert (b["x"], b["y"]) == (0.0, 4.0)

        # Reset scene and flip Y.
        sc._loaded_scene_source_data = {
            "entities": [
                {"id": "a", "prefab_id": "slime_blob", "x": 0.0, "y": 0.0},
                {"id": "b", "prefab_id": "slime_blob", "x": 2.0, "y": 4.0},
        ...
        sc._loaded_scene_data = copy.deepcopy(sc._loaded_scene_source_data)
        capsys.readouterr()

        assert input_capture.handle_key_press(controller, arcade.key.J, arcade.key.MOD_CTRL) is True
        assert capsys.readouterr().out.strip() == "ENTITY_FLIP_Y ok count=2"
        entities = sc.get_authored_scene_payload()["entities"]
        a = next(e for e in entities if e["id"] == "a")
        b = next(e for e in entities if e["id"] == "b")
        assert (a["x"], a["y"]) == (0.0, 4.0)
