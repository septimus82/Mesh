...
def test_release_contract_repo_root_artifacts_absolute(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    args = argparse.Namespace(artifacts=str(artifacts_dir), repo_root=str(repo_root))
    rc = release_contract_command(args)
    out = capsys.readouterr().out

    assert rc == 0
    assert "[Mesh][Release] DONE OK" in out
    log_path = artifacts_dir / "content_contract.log"
