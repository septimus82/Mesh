"""Integration tests for Episode 01 narrative vertical slice."""
...
def _build_episode_context(*, start_quest: bool = True) -> dict[str, Any]:
    ...
    if start_quest:
        assert quests.start_quest("episode_01") is True

    intro_runner = _build_cutscene_runner(window, _load_cutscene_script("ep01_intro"))
    outro_runner = _build_cutscene_runner(window, _load_cutscene_script("ep01_outro"))
...
def _drain_and_route(ctx: dict[str, Any]) -> list[Any]:
    ...
    for event in events:
        ...
        if event.event_type == "ep01_intro_start" and not ctx["intro_runner"].is_running:
            ctx["intro_runner"].start()
        elif event.event_type == "ep01_outro_start" and not ctx["outro_runner"].is_running:
...
def _interact(ctx: dict[str, Any], interactable_id: str) -> list[Any]:
    player = ctx["entities"]["episode_01_player"]
...
def _run_episode(choice_index: int) -> dict[str, Any]:
    ...
    record()
    _step_on_trigger(
        ctx["trigger_volumes"]["episode_01_entry_trigger"],
        ctx["entities"]["episode_01_player"],
    )
    ...
    mentor_dialogue = ctx["dialogue_runners"]["episode_01_mentor"]
    assert mentor_dialogue.is_running is True
    assert mentor_dialogue.choose(choice_index) is True
    _drain_until_empty(ctx)
    record()

    _interact(ctx, "episode_01_journal")
    record()

    _interact(ctx, "episode_01_exit_door")
    record()
    ...
    quest_state = ctx["quest_runner"].get_quest_state("episode_01")
    return {
        "digests": digests,
        "events": list(ctx["event_log"]),
...
class TestEpisode01Integration:
    def test_episode_01_happy_path_choice_help(self) -> None:
        ctx = _build_episode_context()
        assert len(_load_scene_entities()) <= 20
        ...
        assert any(e.event_type == "ep01_entered" for e in events)
        ...
        _interact(ctx, "episode_01_journal")
        assert _get_flag(ctx, "ep01.exit_unlocked") is True
        assert _get_flag(ctx, "ep01.choice_help") is True
        assert _get_flag(ctx, "ep01.choice_solo") is not True

        door_entity = ctx["entities"]["episode_01_exit_door"]
        ...
        assert _get_flag(ctx, "ep01.complete") is True
        assert any(evt["event_type"] == "ep01_complete" for evt in ctx["event_log"])

    def test_episode_01_happy_path_choice_solo(self) -> None:
        ctx = _build_episode_context()
        ...
        _interact(ctx, "episode_01_journal")
        assert _get_flag(ctx, "ep01.exit_unlocked") is True
        assert _get_flag(ctx, "ep01.choice_solo") is True
        assert _get_flag(ctx, "ep01.choice_help") is not True
    ...
    def test_episode_01_save_restore_mid_cutscene(self) -> None:
        ctx = _build_episode_context()
        _step_on_trigger(
            ctx["trigger_volumes"]["episode_01_entry_trigger"],
            ctx["entities"]["episode_01_player"],
        ...
        _advance(restored, 0.25)
        mentor_dialogue = restored["dialogue_runners"]["episode_01_mentor"]
        assert mentor_dialogue.is_running is True
        assert mentor_dialogue.choose(0) is True
        _drain_until_empty(restored)

        _interact(restored, "episode_01_journal")
        _interact(restored, "episode_01_exit_door")
        _advance(restored, 0.25)

        quest_state = restored["quest_runner"].get_quest_state("episode_01")
        assert quest_state is not None
        assert quest_state.status == "completed"

    def test_episode_01_save_restore_mid_dialogue(self) -> None:
        ctx = _build_episode_context()
        _step_on_trigger(
            ctx["trigger_volumes"]["episode_01_entry_trigger"],
            ctx["entities"]["episode_01_player"],
        ...
        snapshot = _snapshot_context(ctx)
        restored = _restore_context(snapshot)
        restored_dialogue = restored["dialogue_runners"]["episode_01_mentor"]
        assert restored_dialogue.is_running is True
        assert restored_dialogue.current_node == "start"

        assert restored_dialogue.choose(1) is True
        _drain_until_empty(restored)
        _interact(restored, "episode_01_journal")
        _interact(restored, "episode_01_exit_door")
        _advance(restored, 0.25)
    ...
    def test_episode_01_determinism_digest_and_events(self) -> None:
        first = _run_episode(choice_index=0)
        second = _run_episode(choice_index=0)
